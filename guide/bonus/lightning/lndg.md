...
* Enable nginx reverse proxy to route external encrypted HTTPS traffic internally to LNDg

  ```sh
  $ sudo nano /etc/nginx/streams-enabled/lndg-reverse-proxy.conf
  ```
...
* Configure the firewall to allow incoming HTTPS requests
  ...
  ```sh
  $ sudo ufw allow 8890/tcp comment 'allow LNDg SSL'
  $ sudo ufw status
  ```
...
* Make the script executable by user “lndg” and exit the user session
...
* With user admin, create a service file for the `jobs.sh` script
  ...
  ```ini
  # RaspiBolt: systemd unit for LNDg
  # /etc/systemd/system/lndg-jobs.service
...
* Create a timer file to run `jobs.sh` every 60 seconds
  ...
  ```sh
  $ sudo nano /etc/systemd/system/lndg-jobs.timer
  ```
